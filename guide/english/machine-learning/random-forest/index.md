...
### Problem
Decision trees by themselves are prone to **overfitting**. This means that the tree becomes so used to the training data that it has difficulty making decisions for data it has never seen before. In the random forest to prevent overfitting trees which are weak learner are used which need more data to train for learning purpose.
...
- <a href='https://github.com/kwatraakshay/Iris-classfication' target='_blank' rel='nofollow'>Iris classification using random forest and other algorithms</a>
